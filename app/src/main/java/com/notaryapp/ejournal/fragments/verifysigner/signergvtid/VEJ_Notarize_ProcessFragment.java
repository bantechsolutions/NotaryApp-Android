...
public class VEJ_Notarize_ProcessFragment extends Fragment {
    ...
    private boolean VEJ_Notarize_VerifyOkFragment = false;

    private boolean wasRunning;

    public VEJ_Notarize_ProcessFragment(String selectedType, String scanRef) {
        this.selectedType = selectedType;
        this.scanRef = scanRef;
    }
    public VEJ_Notarize_ProcessFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        processView = inflater.inflate(R.layout.fragment_notarize_process, container, false);
...
//        });
        ...
        btnAbort.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(!showDialogue){
                    //proceedDialog("If you cancel now, your account will be charged for the verification attempt. If you wait the full 5 minutes and Notary-App™ is still unable to complete the verification, then your account will not be charged.");
        ...
        /*docSpinner.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
...
//                            getJumioResponse();
                            ...
                            runningNegative = false;
                            loadFragment(new VEJ_Notarize_VerifiFailedRetryFragment(null, null, null, null, ""),"");
                            return;
                        }
                    }
                    if(runningNegative){
    ...
    private void init() {
        ...
        //docSpinner = processView.findViewById(R.id.docSpinner);
        btnAbort.setText("CANCEL");

        footerView = processView.findViewById(R.id.footerView);
//        btnClose = processView.findViewById(R.id.btn_pro_close);
        initIJsonListener();
    ...
    private void loadFragment(Fragment fragment, String TAG) {
        ...
        if(TAG.equals("Notarize_VerifyOkFragment")){
            if(VEJ_Notarize_VerifyOkFragment) {
                VEJ_Notarize_VerifyOkFragment = false;
                if (getActivity() != null) {
    ...
    private void getJumioResponse() {
        ...
        try {
            JumioAPIRequest jumioapiRequest = new JumioAPIRequest();
...
//            HashMap<String, String> params = new HashMap<>();
            ...
            //   String url  = "";
            //  jumioapiRequest.request(getActivity(), iJsonListener, params, url, "Jumio");
            if(getActivity() != null) {
        ...
        iJsonListener = new IJsonListener() {
                        ...
                        if (type.equals("Jumio")) {
                            ...
                            try {
                                ...
                                if(documentStatus.equals("APPROVED_VERIFIED")) {
                                    ...
                                    {
                                        VEJ_Notarize_VerifyOkFragment = true;
                                        loadFragment(new VEJ_Notarize_VerifyOkFragment(firstName, lastName, scanRef),"VEJ_Notarize_VerifyOkFragment");
                                        //  loadFragment(new Notarize_VerifyOkFragment(documentStatus, transactionStatus, verificationStatus, docExpiryStatus));
                                        //   getActivity().onBackPressed();
                                    } else {
                                        String retry = "NO";
                                        ...
                                        loadFragment(new VEJ_Notarize_VerifiFailedRetryFragment(documentStatus, transactionStatus, verificationStatus, docExpiryStatus, retry),"");
                                        //   getActivity().onBackPressed();
                                    }

                                }else{
                                    loadFragment(new VEJ_Notarize_VerifiFailedRetryFragment(documentStatus, transactionStatus, verificationStatus, docExpiryStatus, "YES"),"");
                                }
                                // }
                            } catch (Exception e) {
                                //e.printStackTrace();
            ...
            public void onFetchFailure(String msg) {
                ...
                if(waitTimeExpired && !runningPositive && !runningNegative) {
                    ...
                    loadFragment(new VEJ_Notarize_VerifiFailedRetryFragment(documentStatus, transactionStatus,
                            verificationStatus, docExpiryStatus, "YES"),"");//if didnt get response from server within i minute
                }
                else{
