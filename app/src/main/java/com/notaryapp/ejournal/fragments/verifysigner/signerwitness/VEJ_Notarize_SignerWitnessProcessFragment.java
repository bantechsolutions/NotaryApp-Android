...
public class VEJ_Notarize_SignerWitnessProcessFragment extends Fragment {
    ...
    private boolean VEJ_Notarize_SignerWitnessVerifyOkFragment0 = false;
    private boolean VEJ_Notarize_SignerWitnessVerifyOkFragment1 = false;

    private boolean wasRunning;

    private String signerEmail = "";

    public VEJ_Notarize_SignerWitnessProcessFragment(String selectedType, String scanRef) {
        this.selectedType = selectedType;
        this.scanRef = scanRef;
    }

    public VEJ_Notarize_SignerWitnessProcessFragment(String selectedType, String scanRef, int pending) {
        this.selectedType = selectedType;
        this.scanRef = scanRef;
        this.pending = pending;
    }
    public VEJ_Notarize_SignerWitnessProcessFragment(String selectedType, String scanRef, int pending, String signerEmail) {
        this.selectedType = selectedType;
        this.scanRef = scanRef;
        this.pending = pending;
    ...
    public VEJ_Notarize_SignerWitnessProcessFragment() {
...
//                            getJumioResponse();
                            ...
                            runningNegative = false;
                            runningPositive = false;
                            loadFragment(new VEJ_Notarize_SignerWitnessVerifiFailedRetryFragment(null,
    ...
    private void loadFragment(Fragment fragment, String TAG) {
        ...
        if (TAG.equals("Notarize_SignerWitnessVerifyOkFragment0")) {
            if (VEJ_Notarize_SignerWitnessVerifyOkFragment0) {
                VEJ_Notarize_SignerWitnessVerifyOkFragment0 = false;
                if (getActivity() != null) {

                    runningPositive = false;
                    runningNegative = false;
                    runTimer(); // stopRunnable
                    FragmentViewUtil.replaceFragment(getActivity(), VEJ_VerifySignerActivity.REF_VIEW_CONTAINER, fragment, false);

                }
            } else if (!VEJ_Notarize_SignerWitnessVerifyOkFragment1) {
                VEJ_Notarize_SignerWitnessVerifyOkFragment1 = false;
                if (getActivity() != null) {
                            ...
                            try {
                                ...
                                if (documentStatus.equals("APPROVED_VERIFIED")) {
                                    ...
                                    if (transactionStatus.equalsIgnoreCase("DONE") && docExpiryStatus.equalsIgnoreCase("NOT EXPIRED"))
                                    {
                                        if (pending == 1) {
                                            VEJ_Notarize_SignerWitnessVerifyOkFragment0 = false;
                                            VEJ_Notarize_SignerWitnessVerifyOkFragment1 = true;
                                            //Toast.makeText(getActivity(),"111",Toast.LENGTH_LONG).show();
                                            if (((VEJ_VerifySignerActivity)getActivity()).isOpenOne) {
                                                ((VEJ_VerifySignerActivity)getActivity()).isOpenOne = false;
                                                loadFragment(new VEJ_Notarize_SignerWitnessVerifyOkFragment(firstName, lastName, scanRef, 1, signerEmail), "VEJ_Notarize_SignerWitnessVerifyOkFragment0");
                                            } else{


                                                if(getActivity() instanceof VEJ_VerifySignerActivity ){

                                                    VEJ_Notarize_SignerWitnessVerifyOkFragment0 = true;
                                                    VEJ_Notarize_SignerWitnessVerifyOkFragment1 = false;

                                                    if (((VEJ_VerifySignerActivity)getActivity()).isOpenTwo) {
                                                        ((VEJ_VerifySignerActivity)getActivity()).isOpenTwo = false;
                                                        loadFragment(new VEJ_Notarize_SignerWitnessVerifyOkFragment(firstName, lastName, scanRef, signerEmail), "VEJ_Notarize_SignerWitnessVerifyOkFragment0");
                                                    }
                                                }else if(getActivity() instanceof CredibleWitnessAddSelfieActivity){

                                                    VEJ_Notarize_SignerWitnessVerifyOkFragment0 = true;
                                                    VEJ_Notarize_SignerWitnessVerifyOkFragment1 = false;
                                                    if (((VEJ_CredibleWitnessAddSelfieActivity)getActivity()).isOpenTwo) {
                                                        ((VEJ_CredibleWitnessAddSelfieActivity)getActivity()).isOpenTwo = false;
                                                        loadFragment(new VEJ_Notarize_SignerWitnessVerifyOkFragment(firstName, lastName, scanRef, signerEmail), "VEJ_Notarize_SignerWitnessVerifyOkFragment0");
                                                    }
                                            ...
                                            }else if(getActivity() instanceof VEJ_CredibleWitnessAddSelfieActivity){
                                                if (((VEJ_CredibleWitnessAddSelfieActivity)getActivity()).isOpenTwo) {
                                                    ((VEJ_CredibleWitnessAddSelfieActivity)getActivity()).isOpenTwo = false;
                                                    loadFragment(new VEJ_Notarize_SignerWitnessVerifyOkFragment(firstName, lastName, scanRef, signerEmail), "VEJ_Notarize_SignerWitnessVerifyOkFragment0");
                                                }
                                        ...
                                        loadFragment(new VEJ_Notarize_SignerWitnessVerifiFailedRetryFragment(documentStatus, transactionStatus, verificationStatus, docExpiryStatus, retry, signerEmail), "");
                                    }
//
                                } else {
                                    loadFragment(new VEJ_Notarize_SignerWitnessVerifiFailedRetryFragment(documentStatus, transactionStatus, verificationStatus, docExpiryStatus, "", signerEmail), "");
            ...
            public void onFetchFailure(String msg) {
                ...
                if (waitTimeExpired && !runningPositive && !runningNegative) {
                    ...
                    loadFragment(new VEJ_Notarize_SignerWitnessVerifiFailedRetryFragment(documentStatus, transactionStatus, verificationStatus, docExpiryStatus, "YES", signerEmail), "");
                } else {
